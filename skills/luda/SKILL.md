...
- **Outcomes:** read `effect` and the named verification. `desktop_choose` verifies selection, not activation or application of the choice. If the task requires applying or opening it, use an advertised activation or Apply action when needed, then verify the resulting application state. `verified` verifies that condition at the time observed; `dispatched` only means input was sent; `uncertain` means an effect may already have happened. Inspect before retrying uncertain work. A timeout is not permission to repeat Save, Send, Delete, or input.
