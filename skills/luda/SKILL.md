...
- **Selection:** `desktop_choose` verifies selection only. Selection prepares a target; activation acts on it. For a selection-only task, stop once selection is verified. Otherwise check the requested application effect first: some controls apply changes on selection. If the effect is absent, use an advertised activation action or Apply/Open control, then verify the outcome.
