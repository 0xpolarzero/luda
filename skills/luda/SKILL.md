...
- **Shared desktop:** use the same tools without choosing an input mode or routinely activating first. Supported native semantic actions operate in the background; native pointer and keyboard actions use Luda’s own input devices and focus automatically. A distinct agent cursor appears when available. Applications can still change shared windows or focus; inspect the result. There is no fallback to the human mouse/keyboard. A human can act between checks; changed layout or a focus-race error requires fresh grounding. `desktop_control` pauses cooperating Luda clients, not external input programs. Never resume a user-requested pause without their instruction.
