...
Window queries filter title/class. Each paginated call is a new enumeration, so do not assume a frozen list. Use actual identity and context when multiple windows match. Routine activation is unnecessary: native semantics can operate in the background and Luda automatically chooses independent input or ordinary foreground control for the target. Explicit activation reveals a covered target; observe again afterward. The window list’s active flag describes the window manager’s foreground state; independent agent focus can differ.
