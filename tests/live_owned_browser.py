...
async def main(executable):
    ...
    browser_pid=None;profile=None;human=None
    try:
        async with stdio_client(params) as streams:
            async with ClientSession(*streams) as session:
                ...
                while True:
                    ...
                    human_window=next((w for w in windows if w['pid']==human.pid),None)
                    if human_window:break
                ...
                graphemes='A👩🏽‍💻B éC'
                for case,start,end in [('accent',8,9),('zwj',2,4),('accent-caret',8,8),('zwj-caret',2,2),('base-before-accent',7,8),('delete-zwj',2,4),('delete-accent',8,9)]:
                    ...
                    refused=await error('desktop_type','UNSUPPORTED_TEXT_BOUNDARY',element_id=eid,text='' if case.startswith('delete-') else 'x')
                ...
                frame_target=await field('Exact field')
                ...
                frame_before=await fresh_oracle()
                frame_tree=await call('desktop_inspect',window_id=wid)
                record('frame-scope-keeps-native-inspection',frame_tree.get('available') is True and bool(frame_tree['nodes']) and frame_tree['window_id']==wid and frame_tree.get('text_fields')==[] and frame_tree.get('owned_browser')=={'available':False,'code':'BROWSER_SCOPE_UNSUPPORTED'},frame_tree)
                blocked_read=await error('desktop_read_text','BROWSER_SCOPE_UNSUPPORTED',element_id=frame_target)
                blocked_type=await error('desktop_type','BROWSER_SCOPE_UNSUPPORTED',element_id=frame_target,text='must not enter a frame')
                frame_after=await fresh_oracle()
                record('frame-scope-owned-input-refused-without-effect',blocked_read['effect']=='none' and blocked_type['effect']=='none' and all(frame_after[key]==frame_before[key] for key in ('text','selection','events','oracle_document')),{'before':frame_before,'after':frame_after})
        deadline=time.monotonic()+5
        ...
        if human is not None:
            human.terminate()
            try:human.wait(timeout=3)
            except subprocess.TimeoutExpired:human.kill();human.wait(timeout=3)
