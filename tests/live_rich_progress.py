...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'artifacts/rich-progress'
ASSETS=ROOT/'tests/fixtures/owned-rich-editor'
...
async def main(executable):
    ...
    class Handler(http.server.BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            elif self.path=='/bridge.mjs':body=(ROOT/'integrations/prosemirror/luda-prosemirror.mjs').read_bytes();mime='text/javascript'
    ...
    try:
        for transport in ('native','clipboard'):
            ...
            with patch.dict(os.environ,LUDA_CHROMIUM_EXECUTABLE=executable,GTK_IM_MODULE='gtk-im-context-simple'):
                client=await wire.Client(transport).start()
            async def call(tool,**arguments):
                result=(await client.request('tools/call',{'name':tool,'arguments':arguments}))['result']
                return result,json.loads(result['content'][0]['text'])
            try:
                _,opened=await call('desktop_open_browser',url=f'http://127.0.0.1:{service.server_port}/',lifetime='temporary_session');wid=opened['window_id']
                ...
                _,tree=await call('desktop_inspect',window_id=wid,name='Observed rich editor')
                eid=tree['text_fields'][0]['element_id']
                response,error=await call('desktop_type',element_id=eid,text='FIRST_PRIVATE\nSECOND_PRIVATE\nTHIRD_PRIVATE',line_breaks='paragraph',transport=transport)
                ...
                _,status=await call('desktop_status');_,report=await call('desktop_report')
