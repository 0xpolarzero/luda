...
async def main():
 ...
 try:
  ...
  before=state()['selected_ids'];await button('Open focus observer');await until(lambda:state().get('focus_window'));denial=await denied('desktop_choose',{'FOCUS_CHANGED'},element_id=table['Record 0001'],range_end_id=table['Record 0003']);record('focus-loss-no-change',denial['effect']=='none' and state()['selected_ids']==before);await call('desktop_activate',window_id=wid)
