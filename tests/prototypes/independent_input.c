...
int main(int argc,char **argv){
 ...
 if(!strcmp(argv[1],"add")){XIAddMasterInfo a={XIAddMaster,"luda-probe",True,True};XIChangeHierarchy(d,(XIAnyHierarchyChangeInfo*)&a,1);XSync(d,False);return 0;}
 ...
 if(!strcmp(argv[1],"type")){XISetFocus(d,mk,strtoul(argv[2],NULL,0),CurrentTime);XDevice*k=XOpenDevice(d,sk);int key=XKeysymToKeycode(d,XK_a);XTestFakeDeviceKeyEvent(d,k,key,True,NULL,0,0);XTestFakeDeviceKeyEvent(d,k,key,False,NULL,0,0);XCloseDevice(d,k);}
