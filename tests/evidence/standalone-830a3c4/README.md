...
Wheel/source packaging at `301a535` separately matched all 50 modules and skill, and excluded the removed integration and historical reports from the source archive. `package.json` retains artifact hashes; `package-verification.py CHECKOUT FRESH_OUTPUT LOCKED_BUILD_PYTHON` reproduces the checks. This later commit changes evidence/documentation and registration labeling, not runtime or skill bytes. `install.log.gz` is the successful actual build log; `install-setup-failure.log.gz` preserves the earlier missing-parent invocation failure.
