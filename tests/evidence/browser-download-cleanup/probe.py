...
import argparse
parser=argparse.ArgumentParser(description='Private desktop attachment cleanup after three real process failures')
parser.add_argument('--repo',type=Path,required=True)
parser.add_argument('--output',type=Path,required=True)
args=parser.parse_args();REPO=args.repo.resolve()
if os.getuid()==0 or os.environ.get('LUDA_ISOLATED_TEST_DISPLAY')!='1':raise SystemExit('Require private ordinary-UID Xvfb/D-Bus session')
sys.path.insert(0,str(REPO/'tests'))
import live_mcp_disconnect as wire
ROOT=args.output.resolve();ROOT.mkdir(parents=True,exist_ok=True);wire.OUT=ROOT;wire.ROOT=REPO
...
async def main():
 ...
 files=['browser.py','_browser_guard.py','_browser_worker.py']; hashes={name:hashlib.sha256((REPO/'src/luda'/name).read_bytes()).hexdigest() for name in files}
