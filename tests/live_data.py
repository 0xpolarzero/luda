...
with (OUT/'fixture.log').open('w') as log:
 ...
 try:
  ...
  if 'edit' in cell.get('actions',[]):
   d.element(cell['element_id'],'invoke',action='edit');time.sleep(.2)
   ...
   else:record('editable-cell-commit',False,reason='No unique editable child after edit action',entries=entries)
  ...
  if bounds:
   ...
   fresh=inspect(limit=500);showing=[n for n in fresh['nodes'] if n['name'].startswith('Record ') and 'showing' in n['states']]
   if showing:
