...
class InputGuardTests(unittest.TestCase):
    ...
    def test_normal_release_awaits_owned_companion(self):
        with patch('luda.input_guard.run',return_value=b'{"server_generation":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}') as plan,patch('luda.input_guard.subprocess.Popen',side_effect=self.fake_guard({'done':True,'cleanup_verified':True})):
            with held_button('1'):pass
        self.assertTrue(json.loads(plan.call_args.kwargs['data'])['hold'])
    def test_original_error_preserved_after_verified_cleanup(self):
        with patch('luda.input_guard.run',return_value=b'{"server_generation":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}'),patch('luda.input_guard.subprocess.Popen',side_effect=self.fake_guard({'done':True,'cleanup_verified':True})),self.assertRaises(DesktopError) as caught:
    ...
    def test_unverified_release_retains_guardian_before_return(self):
        ...
        with patch('luda.input_guard.run',return_value=b'{"server_generation":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}'),patch('luda.input_guard.subprocess.Popen',side_effect=self.fake_guard(result)),patch('luda.input_guard._retain_guardian') as retain,self.assertRaises(DesktopError) as caught:
            with held_button('1'):pass
    ...
    def test_replacement_cleanup_is_explicit(self):
        ...
        with patch('luda.input_guard.run',return_value=b'{"server_generation":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}'),patch('luda.input_guard.subprocess.Popen',side_effect=self.fake_guard(result)),self.assertRaises(DesktopError) as caught:
            with held_button('1'):pass
