...
The [704-test snapshot](UNIT-COVERAGE-DC4DC70.md) adds image matching and accurate pre-spawn storage errors, with matching ordinary-account evidence and 23 live MCP checks. Subsequent targeted tests cover the schema improvement found by a fresh agent. Native recovery now covers registration, removal and update uncertainty; all thirteen integration patches apply together to the pinned Silo source.

The [717/719-test snapshot](UNIT-COVERAGE-F98B139.md) adds the optional owned-browser provider and rectangle-schema regression, with matching root/ordinary fingerprints. Its actual private browser suite passed, including composition refusal and process cleanup faults. All fifteen Silo patches apply together, including browser skill guidance and the responsive registration panel. Rich-editor production support and Mac product acceptance remain outstanding.

The [730-test snapshot](UNIT-COVERAGE-CURRENT.md) adds production cooperating ProseMirror paragraphs with matching root/ordinary fingerprints and actual integrated HTML/rich MCP suites. The sixteen-patch Silo sequence synchronizes the agent guidance. This does not extend support to arbitrary rich editors or complete product acceptance.
