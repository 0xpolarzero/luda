...
## Asynchronously recycled rows

[The separate recycled-row fixture](RECYCLED-ROWS-QUALIFICATION.md) now verifies
scroll/reinspect/choose against the same native provider paths reused for delayed
pages, with independent stable record-ID oracles, duplicate visible labels,
filter changes, end-of-data and bounded no-progress handling. This supplements
the original 1,200-row viewport fixture; it does not make provider paths durable
business identifiers or prove absence from a stalled loader.
