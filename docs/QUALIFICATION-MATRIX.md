...
The opt-in `data-entry` suite requires `localedef` and distro locale source files under `/usr/share/i18n` (or `LUDA_TEST_I18N_SOURCE`). It compiles a private German locale; it does not change installed locales or download dependencies. `file-destinations` requires Mousepad and verifies symlink and renamed-path saves against the resulting filesystem. See [data-entry evidence](DATA-ENTRY-QUALIFICATION.md) and [file destinations](FILE-DESTINATION-QUALIFICATION.md).

`image-editor` requires the test-only distro GIMP package; it runs its own private 1200×900 desktop and verifies exported PNG pixels. It is not a production dependency. `session-input` exercises a synthetic registered blocking hint through actual MCP, while `bus-generation` restarts only its own private accessibility bus. `resource-limits` exhausts resources only in owned child processes. See each suite’s qualification document for supported scope.
