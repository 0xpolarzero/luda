...
| application-launch | The fixture checks account identity, literal arguments, dispatched status and bounded launch return (SEC-01/03, ERR-02, PERF-05), with observed launched-window association under WM-09. It is not an SSH attach test or a GTK editor save/reopen workflow. |
| application-services | D-Bus/terminal launch acceptance and timeout handling concern ERR-02/04, with launched/singleton window association under WM-09/10; no SSH attachment occurs. |

The remaining matrix associations were retained after comparison with fixture scope. Some intentionally associate a failing or unsupported case, including browser rich text, Electron non-BMP replacement, protected Firefox input and IME composition. An association therefore cannot be counted as a pass. WM-09/10 associations refer to independently observed launched/singleton windows; dispatched launch acceptance alone does not establish the complete window-readiness criterion.
