...
The original installer reported `installed` and selected a release after root `umask 077` had created the prefix/release directories with mode `0700`. A real `runuser -u silo-desktop -- test -x PREFIX` failed. The initial reproduction used the existing mocked build runner but real directory creation and selection; it did not claim an installed-wheel result.
...
Reproduce as root with existing `silo-desktop`, provisioned Python/venv and normal package access, using fresh paths under an accessible parent:
...
This deliberately creates a real root-owned installation and retains it for inspection; use the managed uninstaller as the prefix owner to remove it. It does not change system packages, desktop configuration, user credentials or Mac/Silo settings. The test explicitly uses `--skip-system`; actual apt provisioning is outside this evidence.
