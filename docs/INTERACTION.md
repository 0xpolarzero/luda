...
The current independent-device backend passes [nine public MCP checks](../tests/evidence/private-input/README.md), including click, wheel, drag, concurrent human typing, popup interaction, and visible feedback. Earlier results below describe the shared-input predecessor and do not independently qualify the new backend.
...
`pointer_popup(owner_window_id, popup_id, snapshot_id, x, y, kind='click', button='left', count=1, direction='down')` supports click, hover and scroll. It requires the token in that snapshot, agent-focused owner identity, unchanged managed-window layout and popup signature, unchanged resolution, and coordinates inside the popup. An isolated native hit test also requires the popup to be the topmost root surface at that point. Parent client bounds do not restrict an authorized popup, allowing context menus and submenus outside the parent. Each mutation remains dispatched until the caller verifies an application result.
