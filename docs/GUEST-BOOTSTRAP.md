...
All path/account/output validation and Silo status preflight precede installation.
...
Stdout is one JSON result; installer progress goes to stderr. Arbitrary status
fields, provider stdout and exception text are not copied into the JSON error.
The response reports `stage`, `installation_completed`,
...
Successful readiness is a point-in-time check, not permanent session availability.
...
Seven deterministic tests cover rejection before child execution, stopped/missing
...
status/error redaction and machine-readable CLI failure. Existing installer tests
