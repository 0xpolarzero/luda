...
class Worker:
    def __init__(self, profile):
        ...
        self.effect = 'none'
        self.progress = None
        from ._browser_input import NativeInput
    ...
    def dispatch(self, request):
        ...
        if op=='type':return self.dispatch_type(request)
        raise Refused('UNSUPPORTED_ACTION')

    def dispatch_type(self, request):
        return self.type(request.get('token'),request['text'],request['mode'])

    def read_value(self, item, secret=False):
        if secret:
            from ._browser_secret import SNAPSHOT as snapshot
        else:
            snapshot = SNAPSHOT
        return item['node'].evaluate(snapshot)
    ...
    def send_key(self, key, code, number, modifiers=0):
        self.effect='uncertain'
...
def main(worker_class=None):
    worker=(worker_class or Worker)(sys.argv[1])
    try:
        for raw in sys.stdin.buffer:
            ...
            progress=worker.receipt_progress()
