...
def click_button(button,count=1,target=None,position=None,server_generation=None):
...
def move_pointer(x,y,server_generation,target=None):
    """Move without held buttons, bound to the preflight's original server."""
    return _move_pointer(x,y,server_generation,target=target)


def _move_pointer(x,y,server_generation,target=None,held_button=None):
