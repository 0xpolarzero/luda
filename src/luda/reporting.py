...
METHODS = frozenset('doctor list_windows window_overview recording observe ocr match_image inspect workspaces wait_for wait_condition reconnect recover_input list_applications launch_application open_browser activate pointer pointer_popup hover drag_between key paste element type_text manage_window switch_workspace'.split())
# Deliberate fixed vocabulary; never infer safety from arbitrary uppercase text.
CODES = frozenset('VALUE_UNVERIFIABLE UNSUPPORTED_TEXT_BOUNDARY TEXT_BOUNDARY_UNAVAILABLE BROWSER_SELECTION_CHANGED LINE_BREAK_SEMANTICS_REQUIRED FORMATTING_CHANGED BROWSER_CLEANUP_UNCONFIRMED BROWSER_ADAPTER_UNAVAILABLE BROWSER_CLOSED BROWSER_TIMEOUT BROWSER_PROTOCOL_ERROR BROWSER_ALREADY_OPEN BROWSER_SCOPE_UNSUPPORTED BROWSER_OPERATION_FAILED UNSUPPORTED_FIELD COMPOSITION_UNKNOWN IME_COMPOSITION_ACTIVE MATCH_UNAVAILABLE MATCH_FLAT_TEMPLATE MATCH_FAILED MATCH_LIMIT MATCH_SCALE_MISMATCH RECORDING_BUSY RECORDING_LIMIT RECORDING_UNAVAILABLE RECORDING_FAILED OCR_UNAVAILABLE OCR_LANGUAGE_UNAVAILABLE OCR_INVALID_OUTPUT OCR_LIMIT ACCESSIBILITY_ERROR ACCESSIBILITY_UNAVAILABLE AMBIGUOUS_ACCESSIBILITY_WINDOW INVALID_SELECTION LAUNCH_FAILED NOT_PROTECTED_FIELD SELECTION_TOO_LARGE SELECTION_UNVERIFIABLE TARGET_IDENTITY_UNAVAILABLE UNSUPPORTED_OPERATION ACTION_REQUIRED ACTIVATION_FAILED ADMISSION_UNAVAILABLE BACKEND_ERROR BUSY CANCELLED CLIPBOARD_CHANGED CLIPBOARD_FAILED CLOSED CONTROL_PAUSED CONTROL_UNAVAILABLE DEPENDENCY_MISSING DESKTOP_CHANGED DISPLAY_UNAVAILABLE FOCUS_CHANGED FOCUS_UNVERIFIED INPUT_CLEANUP_PENDING INPUT_HELD INPUT_RELEASE_UNVERIFIED INPUT_STATE_CHANGED INPUT_UNAVAILABLE INTERNAL_ERROR INVALID_ARGUMENT INVALID_CARET INVALID_KEY INVALID_PROPERTY INVALID_WINDOW_TOKEN KEYBOARD_CLEANUP_PENDING KEYBOARD_UNAVAILABLE KEYMAP_CHANGED NOT_EDITABLE NOT_INTERACTABLE OCCLUDED_TARGET OUTPUT_LIMIT OUT_OF_BOUNDS PROTECTED_FIELD RESOURCE_UNAVAILABLE SCREENSHOT_LIMIT SCREENSHOT_UNAVAILABLE SELECTION_UNVERIFIED SESSION_BLOCKED SESSION_CHANGED STALE_OBSERVATION STALE_TARGET TEXT_CHANGED TEXT_MISMATCH TEXT_REPRESENTATION_UNSUPPORTED TEXT_TOO_LARGE TIMEOUT TOPOLOGY_UNAVAILABLE UNSAFE_RUNTIME UNSUPPORTED UNSUPPORTED_BACKEND UNSUPPORTED_ACTION UNSUPPORTED_INPUT_STATE UNSUPPORTED_KEYMAP UNSUPPORTED_PASTE UNSUPPORTED_SELECTION UNSUPPORTED_TEXT VERIFICATION_LIMIT'.split())
