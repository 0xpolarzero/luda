...
def decode(value):
    ...
    if len(json.dumps(model,ensure_ascii=False).encode())>512000:
        raise RichInvalid('VERIFICATION_LIMIT')
    texts=[]; styled=[]; offsets={};position=0;logical=0;nodes=0
    for index,paragraph in enumerate(paragraphs):
        ...
        for node in content:
            ...
            if not isinstance(node,dict) or node.get('type')!='text' or set(node)-{'type','text','marks'} or not isinstance(node.get('text'),str):
                raise RichInvalid('TEXT_REPRESENTATION_UNSUPPORTED')
            ...
            for char in node['text']:
                ...
                text+=char;marks.append(tuple(m['type'] for m in formatting));native+=2 if ord(char)>0xFFFF else 1
        ...
        position=native+1
        if index<len(paragraphs)-1:styled.append(None);logical+=1
    if logical>64000:raise RichInvalid('VERIFICATION_LIMIT')
    ...
    return {**value,'text':'\n'.join(texts),'styled':styled,'start':start,'end':end,
            ...
            'paragraphs':texts,'tag':'PROSEMIRROR','type':'basic-paragraphs-v1'}
