...
ROOT=Path(__file__).resolve().parents[3];OUT=ROOT/'artifacts/owned-hard-breaks';ASSETS=ROOT/'addons/editor-bridge/tests/fixtures/owned-rich-hard-breaks'
def units(model):
 result=[]
...
async def main(executable):
 ...
 class Handler(http.server.BaseHTTPRequestHandler):
  ...
  def do_GET(self):
   name=self.path.split('?')[0];p=ROOT/'addons/editor-bridge/application/luda-prosemirror.mjs' if name=='/bridge.mjs' else ASSETS/('index.html' if name=='/' else name.lstrip('/'))
   if p.parent not in (ASSETS,ROOT/'addons/editor-bridge/application') or not p.is_file():self.send_error(404);return
 ...
 with patch.dict(os.environ,LUDA_CHROMIUM_EXECUTABLE=executable,GTK_IM_MODULE='gtk-im-context-simple'):
  ...
  try:
   opened=await call('editor_open',url=f'http://127.0.0.1:{service.server_port}',lifetime='temporary_session');wid=opened['window_id'];await call('editor_activate',window_id=wid);await wait(lambda:'model' in state)
   async def field():
    tree=await call('editor_inspect',window_id=wid,name='Observed rich editor',role='entry');f=tree['text_fields'];assert len(f)==1;return f[0]
   async def button(label):
    tree=await call('editor_inspect',window_id=wid,name=label);n=next(n for n in tree['nodes'] if n['name']==label and n['role']=='push button');assert 'press' in n['actions'];await call('editor_invoke',element_id=n['element_id'],action='press')
   ...
   for transport in ['native','clipboard']:
    for name,payload in [('unicode','A😀 é 日本語'),('spaces','  a\t b\u00a0c  '),('consecutive','a\n\nb'),('trailing','a\n\n'),('only-breaks','\n\n')]:
     eid=await fresh();r=await call('editor_type',element_id=eid,text=payload,mode='replace',line_breaks='hard_break',transport=transport);await wait(lambda:state['modelText']==payload)
     ...
     read=await call('editor_read',element_id=eid);record(transport+'-'+name+'-caret',read['caret_offset']==len(payload) and state['modelCaretCodePoints']==len(payload) and read['line_break_boundaries']==[{'offset':i,'kind':'hard_break'} for i,c in enumerate(payload) if c=='\n'])
     if '\n' in payload:
   ...
   eid=await fresh('Mixed marked document');before=json.loads(json.dumps(state['model']));read=await call('editor_read',element_id=eid);record('mixed-structure-read',read['text']=='A😀\néZ\nTAIL' and read['line_break_boundaries']==[{'offset':2,'kind':'hard_break'},{'offset':6,'kind':'paragraph'}],read=read,oracle=dict(state))
   await call('editor_focus',element_id=eid);await call('editor_select',element_id=eid,start_offset=len(read['text']),end_offset=len(read['text']));r=await call('editor_type',element_id=eid,text='\nEND',line_breaks='hard_break');await wait(lambda:state['model']==r['model']);record('native-append-preserves-mixed-prefix',units(r['model'])[:len(units(before))]==units(before),result=r,oracle=dict(state))
   for policy,start,end in [('hard_break',1,5),('paragraph',2,7),('hard_break',2,3),('hard_break',6,7),('hard_break',3,3)]:
    eid=await fresh('Mixed marked document');before=json.loads(json.dumps(state['model']));old=units(before);await call('editor_focus',element_id=eid);await call('editor_select',element_id=eid,start_offset=start,end_offset=end)
    r=await call('editor_type',element_id=eid,text='X\n😀',transport='clipboard',line_breaks=policy);await wait(lambda:state['model']==r['model']);actual=units(state['model']);inserted=[('X','text'),('\n',policy),('😀','text')]
   ...
   eid=await fresh('Mixed marked document');r=await call('editor_type',element_id=eid,text='FULL\nREPLACE\n',mode='replace',line_breaks='hard_break');await wait(lambda:state['model']==r['model']);record('native-full-replace-mixed',len(state['model']['content'])==1 and state['modelText']=='FULL\nREPLACE\n',result=r,oracle=dict(state))
   eid=await fresh();before=json.loads(json.dumps(state['model']));v=await deny('editor_type','LINE_BREAK_SEMANTICS_REQUIRED',element_id=eid,text='no\nimplicit');record('missing-policy-no-mutation',v['effect']=='none' and state['model']==before)
   eid=await fresh('Legacy editor');before=json.loads(json.dumps(state['model']));v=await deny('editor_type','UNSUPPORTED_ACTION',element_id=eid,text='x\ny',line_breaks='hard_break');record('legacy-policy-no-mutation',v['effect']=='none' and state['model']==before)
   eid=await fresh('Legacy mixed document');before=json.loads(json.dumps(state['model']));v=await deny('editor_type','TEXT_REPRESENTATION_UNSUPPORTED',element_id=eid,text='no replacement',mode='replace');record('legacy-mixed-no-mutation',v['effect']=='none' and state['model']==before)
   eid=await fresh('Mixed marked document');await button('Insert image model node');await wait(lambda:'image' in state['unsupported']);before=json.loads(json.dumps(state['model']));v=await deny('editor_type','TEXT_REPRESENTATION_UNSUPPORTED',element_id=eid,text='no mixed replacement',mode='replace',line_breaks='hard_break');record('unsupported-mixed-no-mutation',v['effect']=='none' and state['model']==before)
   eid=await fresh('Wrong break binding');v=await deny('editor_type','TEXT_MISMATCH',element_id=eid,text='A\nB',line_breaks='hard_break');await wait(lambda:state['modelText']=='A\n');record('wrong-binding-stops-no-replay',v['effect']=='uncertain' and len(state['model']['content'])==2 and state['modelText']=='A\n',oracle=dict(state))
   eid=await fresh();await button('Move focus after next input');v=await deny('editor_type','FOCUS_CHANGED',element_id=eid,text='FIRST\nSECOND',line_breaks='hard_break');await wait(lambda:state['modelText']=='FIRST');record('focus-change-stops',v['effect']=='uncertain' and state['modelText']=='FIRST',oracle=dict(state))
   eid=await fresh();await call('editor_focus',element_id=eid);await call('editor_press_keys',window_id=wid,chord='ctrl+shift+u')
   for digit in ['3','0','6','b']:await call('editor_press_keys',window_id=wid,chord=digit)
   await wait(lambda:any(e['type']=='compositionstart' and e['trusted'] for e in state['events']))
   pending=await call('editor_read',element_id=eid);before=json.loads(json.dumps(state['model']));record('native-preedit-observed',pending['composition']['known'] and pending['composition']['active'],read=pending,oracle=dict(state))
   for transport in ['native','clipboard']:
    v=await deny('editor_type','IME_COMPOSITION_ACTIVE',element_id=eid,text='NO\nINPUT',line_breaks='hard_break',transport=transport);after=await call('editor_read',element_id=eid);record(transport+'-preedit-refusal-preserves',v['effect']=='none' and state['model']==before and after['composition']['active'] and after['text']==pending['text'],response=v,oracle=dict(state))
