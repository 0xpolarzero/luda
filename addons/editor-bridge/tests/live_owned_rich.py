...
ROOT=Path(__file__).resolve().parents[3];OUT=ROOT/'artifacts/owned-rich';ASSETS=ROOT/'addons/editor-bridge/tests/fixtures/owned-rich-editor'
    ...
    with patch.dict(os.environ,LUDA_CHROMIUM_EXECUTABLE=executable,GTK_IM_MODULE='gtk-im-context-simple'):
        ...
        try:
            ...
            for name,payload in SAMPLES:
                ...
                eid=await fresh()
                result=await call('editor_type',element_id=eid,text=payload,mode='replace',line_breaks='paragraph')
            ...
            await call('editor_type',element_id=eid,text='BOLD 😀')
            old_model=json.loads(json.dumps(state['model']))
            result=await call('editor_type',element_id=eid,text='\nplain\n',line_breaks='paragraph')
            ...
            await denied('editor_type','LINE_BREAK_SEMANTICS_REQUIRED',element_id=eid,text='a\nb')
            record('missing-policy-no-mutation',state['model']==before['model'])
            await call('editor_press_keys',window_id=wid,chord='Left')
            await denied('editor_type','UNSUPPORTED_SELECTION',element_id=eid,text='no mid insertion')
            previous=eid;eid=await fresh();await denied('editor_type','STALE_TARGET',element_id=previous,text='stale')
            await button('Insert image model node')
            await denied('editor_type','TEXT_REPRESENTATION_UNSUPPORTED',element_id=eid,text='no image replacement',mode='replace')
            eid=await fresh();await button('Arm unsupported link mark')
            pending_mark=await denied('editor_type','TEXT_REPRESENTATION_UNSUPPORTED',element_id=eid,text='must not insert')
            ...
            await button('Renew registration');await denied('editor_read','STALE_TARGET',element_id=eid)
            ...
            preedit=await call('editor_read',element_id=eid)
            ...
            await denied('editor_type','IME_COMPOSITION_ACTIVE',element_id=eid,text='do not replace preedit',mode='replace')
            ...
            pending=await client.begin('tools/call',{'name':'editor_type','arguments':{'element_id':eid,'text':payload,'line_breaks':'paragraph'}})
            ...
            eid=await field();await call('editor_type',element_id=eid,text='explicit recovery')
            ...
            await call('editor_select',element_id=eid,start_offset=127,end_offset=127)
            before_limit=json.loads(json.dumps(state['model']))
            refusal=await denied('editor_type','VERIFICATION_LIMIT',element_id=eid,text='\n',line_breaks='paragraph')
