    ...
    try:
        ...
        while not (base/'stop').exists() and time.monotonic()<end:
            if task == 'theme':
                ...
                pending=[Atspi.get_desktop(0)]; selected=[]; seen=0
                ...
                state={'selected':selected,'opened':(base/'opened').exists()}
            (base/'oracle.json').write_text(json.dumps(state))
...
def launch(base, themes, task):
    ...
    env.update(XDG_CONFIG_DIRS=env['XDG_CONFIG_HOME'], XDG_DATA_DIRS=f'{themes}:/usr/local/share:/usr/share', XDG_CURRENT_DESKTOP='XFCE', LANG='C.UTF-8', LC_ALL='C.UTF-8', NO_AT_BRIDGE='0', GTK_MODULES='gail:atk-bridge')
...
def main():
    ...
    parser.add_argument('--task',choices=['theme','file'],default='theme'); parser.add_argument('--user',default='ubuntu'); parser.add_argument('--timeout',type=int,default=180)
    ...
    result['source_hashes']={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),ROOT/'skills/luda/SKILL.md',ROOT/'src/luda/ax_worker.py']}
    agent=launcher=None
    with tempfile.TemporaryDirectory(prefix='luda-selection-',dir='/workspace') as directory:
        ...
        try:
            with (out/'desktop.log').open('wb') as log:
                ...
                result['passed']=result.get('returncode')==0 and (result['oracle']['theme']=='Greybird-dark' if args.task=='theme' else bool(result['oracle']['selected']) and not result['oracle']['opened'] and not result['initial_oracle']['selected'])
