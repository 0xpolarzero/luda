    ...
    with tempfile.TemporaryDirectory(prefix='luda-agent-rich-') as directory:
        ...
        try:
            with (out/'desktop.log').open('wb') as log:
                ...
                server_args=['-u','silo-desktop','--','/usr/bin/env',*[k+'='+v for k,v in ready['environment'].items()],str(ROOT/'.venv/bin/python'),str(ROOT/'tests/stale_snapshot_relay.py'),str(desktop),str(ROOT/'.venv/bin/luda')]
