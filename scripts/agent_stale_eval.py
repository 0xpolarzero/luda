#!/usr/bin/env python3
"""One first-attempt stale screenshot recovery task, existing CLI auth, ordinary private GUI."""
...
def grade(actual,moved):
    return {'committed_amber_once':actual.get('selected')=='Amber' and actual.get('commits')==1,
            'only_intended_clicks':[row.get('target') for row in actual.get('clicks',[])]==['Amber','Commit'],
            'external_move_observed':moved.get('moved') is True}
...
def trace_observations(events):
    ...
    stale_index=None;stale_arguments={};new_snapshot=None;recomputed=False
    for index,item in enumerate(calls):
        for content in (item.get('result') or {}).get('content',[]):
            ...
            if value.get('code')=='STALE_OBSERVATION' and value.get('effect')=='none':
                stale_index=index;stale_arguments=item.get('arguments',{})
            if stale_index is not None and index>stale_index and item.get('tool')=='desktop_observe' and value.get('ok') is True:new_snapshot=value.get('snapshot_id')
        arguments=item.get('arguments',{})
        if (new_snapshot and new_snapshot!=stale_arguments.get('snapshot_id') and item.get('tool')=='desktop_click'
                and arguments.get('snapshot_id')==new_snapshot
                and (arguments.get('x'),arguments.get('y'))!=(stale_arguments.get('x'),stale_arguments.get('y'))):recomputed=True
    return {'stale_observation_exposed':stale_index is not None,'reobserved_and_used_new_snapshot':recomputed,
...
def child(base):
    ...
    try:
        ...
        while subprocess.run(['wmctrl','-m'],capture_output=True,timeout=2).returncode:
            ...
            time.sleep(.05)
        fixture=subprocess.Popen(['/usr/bin/python3',str(ROOT/'tests/agent_stale_fixture.py'),str(base)],start_new_session=True)
...
def main():
    ...
    out=ROOT/'artifacts/agent-stale-snapshot'/('run-'+str(time.time_ns()));out.mkdir(parents=True)
    ...
    with tempfile.TemporaryDirectory(prefix='luda-agent-rich-') as directory:
        ...
        try:
            with (out/'desktop.log').open('wb') as log:
                ...
                prompt='In Parcel Board, choose the Amber label and commit it once. Verify the saved result. Use only public Luda desktop MCP tools. Read the installed Luda skill first; you may read .agents/skills/luda/SKILL.md. Do not read application source, hidden files or oracle data, run other programs, modify files directly, or browse websites.'
                result['prompt']=prompt
                server_args=['-u',os.environ.get('LUDA_TEST_USER', 'desktop'),'--','/usr/bin/env',*[k+'='+v for k,v in ready['environment'].items()],str(ROOT/'.venv/bin/python'),str(ROOT/'tests/stale_snapshot_relay.py'),str(desktop),str(ROOT/'.venv/bin/luda')]
                ...
                actual=json.loads((desktop/'board.json').read_text()) if (desktop/'board.json').exists() else {}
                accepted=json.loads((desktop/'moved.json').read_text()) if (desktop/'moved.json').exists() else {}
                ...
                result['workflow_oracle_passed']=result['workflow_oracle_passed'] and result['stale_observation_exposed'] and result['reobserved_and_used_new_snapshot']
