...
The [tool reference](docs/TOOLS.md) is generated from actual MCP declarations. The [skill](skills/luda/SKILL.md) explains targeting, text and recovery. Other capabilities include window/workspace management, cross-window drag, clipboard paste, shared pause/resume and operation status. Pointer and key dispatch still require application-specific verification.
...
The fresh-display runner exercises native controls, actual MCP, cancellation and shared pause. Other live suites cover GTK3, Qt5, GTK4, Chromium, applications, nested menus, cross-window drag, installation and X-server recovery. They use synthetic documents and independent widget/file/DOM readback. See [qualification methodology](docs/QUALIFICATION.md) and [toolkit](docs/TOOLKIT-QUALIFICATION.md) / [browser](docs/BROWSER-QUALIFICATION.md) results for failures as well as passes.
...
Window identities include process lifetime and an X-resource generation token. Screenshot coordinates expire and are revalidated against geometry, focus, menus and the topmost surface. Semantic handles are scoped to an observed window and expire. Input is serialized across cooperating clients, and `desktop_control` can pause their mutations.
